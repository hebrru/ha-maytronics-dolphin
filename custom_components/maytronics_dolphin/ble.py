...
def _ble_device_from_discovered_identity(hass: HomeAssistant, addr: str) -> BLEDevice | None:
    """Match config MAC to an advertiser by BD_ADDR or by local name.

    Many Dolphins advertise a random-looking BD_ADDR while the shortened local
    name carries the TI-style identity bytes, so the user may configure either.
    """
...
async def _wait_for_address_advertisement(
    hass: HomeAssistant, addr: str
) -> BluetoothServiceInfoBleak:
    """Wait for a BLE advertisement from a known on-air address."""
...
async def _wait_for_identity_advertisement(
    hass: HomeAssistant, addr: str
) -> BluetoothServiceInfoBleak:
    """Fallback for configured 12-hex Dolphin identity names."""
    want = _addr_hex_digits(addr)
    return await async_process_advertisements(
        hass,
        lambda si: _has_mydolphin_service(si)
        and (
            _addr_hex_digits(si.address) == want
            or _addr_hex_digits(si.name or "") == want
        ),
...
async def async_resolve_ble_device(hass: HomeAssistant, address: str) -> BLEDevice:
    ...
    if ble_device is None:
        ...
        try:
            service_info = await _wait_for_address_advertisement(hass, addr)
        except TimeoutError:
            try:
                service_info = await _wait_for_identity_advertisement(hass, addr)
            except TimeoutError:
                raise HomeAssistantError(
                    ...
                    "Settings -> Devices & services -> Bluetooth. The device should "
                    "appear there before controls work. Wrong MAC also causes this."
                ) from None
