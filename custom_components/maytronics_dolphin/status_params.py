...
def is_floor_only_marker(snap: InternalParamsSnapshot | None) -> bool:
    ...
    return (
        ...
        and snap.climb_every == FLOOR_ONLY_CLIMB_MARKER
    )


def resolve_clean_mode_key(
    clean_mode: CleanMode | None,
    internal: InternalParamsSnapshot | None,
) -> str | None:
    """Return the app-level clean mode key, including the ``floor_only`` marker."""
    if is_floor_only_marker(internal):
        return "floor_only"
    if clean_mode == CleanMode.REGULAR:
        return "regular"
    if clean_mode == CleanMode.FAST_MODE:
        return "fast_mode"
    ...
    if clean_mode is None:
        return None
    return clean_mode_to_str(clean_mode)
