...
_MAC_OR_NAME_RE = re.compile(
    r"^(?:([0-9A-Fa-f]{2}:){5}([0-9A-Fa-f]{2})|[0-9A-Fa-f]{12})$"
)


def _format_address_or_name(value: str) -> str:
    """Format a real MAC or a 12-hex Dolphin advertised identity."""
    hex_digits = "".join(c for c in value if c in "0123456789abcdefABCDEF")
...
class MaytronicsDolphinConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            if not _MAC_OR_NAME_RE.match(address):
                errors["base"] = "invalid_mac"
            else:
                address_fmt = _format_address_or_name(address)
